...
import {
  ...
  Bot, User, Cpu, Filter, Search, ChevronDown,
} from "lucide-react";
...
const OUTCOME_STYLE: Record<string, { icon: React.ElementType; cls: string; label: string }> = {
...
export default function AuditPage() {
  ...
  const [selected, setSelected] = useState<string | null>(mockAuditLogs[0]?.id ?? null);

  const filtered = mockAuditLogs.filter((l) => {
    if (severityFilter !== "all" && l.severity !== severityFilter) return false;
    if (search) {
      const q = search.toLowerCase();
      return l.actor.toLowerCase().includes(q) || l.action.toLowerCase().includes(q) || l.resource_name.toLowerCase().includes(q);
    }
    return true;
  });

  const detail = mockAuditLogs.find((l) => l.id === selected);
  const warnings = mockAuditLogs.filter((l) => l.severity === "warning" || l.severity === "critical").length;
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    selected === log.id ? "bg-blue-50 ring-1 ring-blue-200" : "hover:bg-slate-50",
                    log.outcome === "blocked" ? "border-l-2 border-l-orange-400 pl-3" :
                    log.severity === "warning" ? "border-l-2 border-l-amber-400 pl-3" : ""
                  )}
                >
                  {/* Icon */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className={cn("inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-semibold ring-1 ring-inset", SEVERITY_STYLE[log.severity])}>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <span className="text-xs text-slate-500">{log.resource_type} / <span className="font-medium text-slate-700">{log.resource_name}</span></span>
                      <span className="text-[11px] text-slate-400">{formatDate(log.timestamp)}</span>
                    </div>
                    {log.reason && (
                      <p className="text-xs text-slate-500 mt-1 italic truncate">{log.reason}</p>
                    )}
                  </div>
                </div>
              );
            ...
            <div className="px-4 py-4 space-y-4">
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  { label: "严重级别", value: SEVERITY_LABEL[detail.severity] },
              ...
              <div>
                ...
                <code className="block text-xs bg-[var(--bg-code)] border border-slate-200 rounded-lg px-3 py-2 font-mono text-blue-700 break-all">{detail.action}</code>
              </div>

              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">目标资源</p>
                <div className="rounded-lg bg-slate-50 border border-slate-100 px-3 py-2">
                  <p className="text-xs text-slate-500">{detail.resource_type}</p>
