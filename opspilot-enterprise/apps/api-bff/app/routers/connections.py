...
async def resolve_connection_context(
    ...
    return {"connection": conn, "credentials": resolved_creds}
...
MOCK_CONNECTIONS: list[dict[str, Any]] = [
    {
        ...
        "enabled": True,
        "version": "vSphere 8.0",
        "description": "生产环境 vCenter Server",
    ...
    },
    {
        "id": "conn-k8s-prod",
        "name": "k8s-prod",
        "display_name": "K8s 生产集群",
        "type": "kubeconfig",
        "category": CATEGORIES["kubeconfig"],
        "endpoint": "https://kubernetes.default.svc",
        "scope": "Namespace: all",
        "credential_ref": "secret://k8s-prod",
        "proxy_config": None,
        "status": "active",
        "enabled": True,
        "version": "K8s",
        ...
        "last_test_result": None,
        "last_test_latency_ms": None,
        "bound_tools": ["k8s.list_nodes", "k8s.list_pods", "k8s.get_workload_status", "k8s.restart_deployment", "k8s.scale_deployment"],
        "tags": ["k8s", "production", "core"],
    },
]
...
async def create_connection(body: CreateConnectionBody):
    ...
    MOCK_AUDITS.insert(
        0,
        {
            ...
            "action": "created",
            "actor": "admin",
            "detail": f"created connection: {body.display_name}",
            "timestamp": _now(),
            "ip": "127.0.0.1",
        },
    )
...
async def toggle_connection(conn_id: str, body: ToggleConnectionBody):
    ...
    MOCK_AUDITS.insert(
        0,
        {
            ...
            "actor": "admin",
            "detail": "enabled connection" if body.enabled else "disabled connection",
            "timestamp": _now(),
            "ip": "127.0.0.1",
        },
    )
...
async def test_connection(conn_id: str, body: Optional[TestConnectionBody] = None):
    ...
    cred_ref = conn.get("credential_ref", "")
    resolved_creds: dict[str, Any] | None = None
    if cred_ref.startswith("secret://"):
        resolved_creds = await resolve_credential(cred_ref)
    ...
    if conn_type == "vcenter":
        ...
        checks = await test_k8s_connection(conn["endpoint"], resolved_creds)
        is_ok = all(c["passed"] for c in checks)
        latency = sum(c["duration_ms"] for c in checks)
    else:
        is_ok = conn["status"] != "error" and conn["enabled"]
        latency = 45 if is_ok else 15000
        checks = [
            {"name": "dns_resolve", "passed": True, "message": "dns resolved", "duration_ms": 2},
            {"name": "tcp_connect", "passed": True, "message": "tcp connected", "duration_ms": 8},
            {"name": "auth", "passed": is_ok, "message": "auth ok" if is_ok else "auth failed", "duration_ms": 15 if is_ok else 100},
            {"name": "api_health", "passed": is_ok, "message": "api healthy" if is_ok else "api timeout", "duration_ms": latency},
        ]

    conn["last_tested"] = _now()
    conn["last_test_result"] = "pass" if is_ok else "fail"
    conn["last_test_latency_ms"] = latency
    conn["status"] = "active" if is_ok else "error"
    conn["updated_at"] = _now()
    ...
    MOCK_AUDITS.insert(
        0,
        {
            ...
            "actor": "admin",
            "detail": f"test {'passed' if is_ok else 'failed'}, latency={latency}ms",
            "timestamp": _now(),
            "ip": "127.0.0.1",
        },
    )
...
async def update_connection(conn_id: str, body: UpdateConnectionBody):
    ...
    next_type = conn["type"]
    next_ref = body.credential_ref if body.credential_ref is not None else conn.get("credential_ref", "")
    ...
    for field in ("display_name", "endpoint", "scope", "credential_ref", "proxy_config", "description", "tags"):
        ...
        if val is not None:
            old = conn.get(field)
            conn[field] = val if field != "proxy_config" else (val or None)
            if old != conn[field]:
    ...
    MOCK_AUDITS.insert(
        0,
        {
            ...
            "action": "updated",
            "actor": "admin",
            "detail": f"updated fields: {', '.join(changes) if changes else 'none'}",
            "timestamp": _now(),
            "ip": "127.0.0.1",
        },
    )
        ...
        if conn["id"] == conn_id:
            ...
            MOCK_AUDITS.insert(
                0,
                {
                    ...
                    "action": "deleted",
                    "actor": "admin",
                    "detail": f"deleted connection: {removed['display_name']}",
                    "timestamp": _now(),
                    "ip": "127.0.0.1",
                },
            )
