...
为支撑新增运维场景（告警治理、异常检测、自动化执行、容量预测等），需新增以下核心页面：

---

### 9.1.1 告警治理中心（Alert Center）
...
### 9.1.2 异常检测中心（Anomaly Center）

#### 页面布局

```text
上：资源筛选（VM / K8s / Network / Storage）
中：时间序列 + 基线曲线
下：异常事件列表
```
...
### 9.1.5 容量与趋势分析页面（Capacity Center）
...
### 9.1.6 巡检与合规中心（Compliance Center）
...
### 9.1.8 运行复盘中心（Postmortem Center）
...
| 页面        | 对应 SubAgent                      |
| --------- | -------------------------------- |
| 告警中心      | Alert Correlation Agent          |
| 异常中心      | Anomaly Detection Agent          |
| 拓扑视图      | Evidence / Topology Agent        |
| Runbook中心 | Recommendation / Execution Agent |
| 容量中心      | Capacity Agent                   |
| 巡检中心      | Compliance Agent                 |
| SLA视图     | Business Analysis Agent          |
| 复盘中心      | Case Archive Agent               |
| 知识中心      | KB / RAG Agent                   |

---

## 9.4 页面与Tool Gateway交互原则
